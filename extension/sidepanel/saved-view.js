...
(function () {
   ...
   *
   * Folder selection is its own map (folder id -> true), held the same way and
   * never derived from the item selection. A folder's checkbox picks the
   * folder AND its items; an empty folder has no items to stand in for it, so
   * the folder itself is what is selected, and that is what lets an empty
   * folder be deleted from the list. The default folder is never in this map:
   * its checkbox picks items only.
   *
  ...
  // The folder that always exists. saved.js owns the id and exports it, but
  // this is a classic script and cannot import, so it is restated here. It is
  // the id, never the name: saved.js lets the default folder be renamed.
  var DEFAULT_FOLDER_ID = "f0";
  ...
  // In folder order, and never the default folder: that one cannot be deleted.
  function selectedFolderIds() {
    return folders
      .filter(function (folder) {
        return folder.id !== DEFAULT_FOLDER_ID && selectedFolders[folder.id] === true;
      })
      .map(function (folder) { return folder.id; });
  }

  // The folders the list shows: every folder under All, the one folder under a
  // filter. Contents never gate this (user-directed): an empty folder is still
  // a folder, and hiding it is what made folders seem to vanish when the last
  // item left.
  ...
  // target without the user having to select first. A checked folder counts as
  // something checked: with only an empty folder picked, an action must not
  // quietly widen to everything shown.
  ...
  function buildBar() {
    ...
    selectAll.addEventListener("change", function () {
      ...
      // Select-all is about items; folders are picked one by one. Clearing it
      // clears the folders too, so the clearing gesture leaves nothing armed
      // behind it.
  ...
  // One sentence for the folder going away, and a second for the items it
  // leaves behind. The destination is read off the default folder rather than
  // written out as "Saved", because saved.js lets that folder be renamed.
  // Shared by the toolbar's per-folder confirmation and the batch one.
  function folderDeleteText(id, moving) {
    var text = "Delete " + folderName(id) + "?";
    if (!moving) return text;
    return text + " " + moving +
      (moving === 1 ? " item moves to " : " items move to ") +
      folderName(DEFAULT_FOLDER_ID) + ".";
  }
  ...
  function openDeleteConfirm() {
    if (!filterId || filterId === DEFAULT_FOLDER_ID) return;
    var box = el("div", "saved-confirm saved-confirm--folder");
    box.appendChild(el("span", "saved-confirm-text",
      folderDeleteText(filterId, itemsInFolder(filterId).length)));
  ...
  function renderBar() {
    ...
    // Rename and Delete are about ONE folder, so they are inert on All; the
    // default folder always exists, so it offers no delete at all.
    els.renameBtn.disabled = !filterId;
    els.deleteBtn.hidden = !filterId || filterId === DEFAULT_FOLDER_ID;
  ...
  function buildFolderHeader(folder) {
    ...
    // Every band is selectable, empty or not: that is how an empty folder gets
    // deleted. The one exception is an empty default folder, which can neither
    // be deleted nor has anything to select, so its box has nothing to do.
    check.disabled = folder.id === DEFAULT_FOLDER_ID && rows.length === 0;
    ...
    check.addEventListener("change", function () {
      ...
      // The folder itself joins the selection, except the default folder,
      // whose box only ever stands for its items: it is never deleted.
      if (check.checked && folder.id !== DEFAULT_FOLDER_ID) {
        selectedFolders[folder.id] = true;
      } else {
        delete selectedFolders[folder.id];
      }
      renderList();
      renderActions();
      renderBar();
  ...
  // The header's checkbox reads checked for a folder picked as a whole, or for
  // one whose every item is picked; part of its items picked reads
  // indeterminate. Shared by the build and the cheap re-sync, so the two can
  // never drift apart.
  ...
  function renderListBody() {
    ...
    // Item rows indent under the folder bands, under a filter as well as under
    // All; the class keeps that a stylesheet concern.
    ...
    // which render regardless, so the folders a user made are still there
    // after the last item leaves them.
    if (!items.length) {
    ...
    for (var f = 0; f < shown.length; f++) {
      ...
      // A collapsed folder shows nothing under its band, its empty line
      // included: the collapse sits above the append.
      if (collapsed[folder.id] === true) continue;
      if (!inFolder.length) {
        list.appendChild(el("p", "saved-empty saved-empty--folder",
  ...
  function openRemoveConfirm() {
    ...
    var no = button("saved-confirm-no", "Cancel");
    yes.addEventListener("click", function () {
      // Items first, folders last: a folder deleted first would move its items
      // to the default folder before the item deletion could reach them. Each
      // write goes through mutate(), so each one claims its own storage event.
      var work = ids.length
        ? mutate({ type: "savedRemove", ids: ids })
        : Promise.resolve(null);
      folderIds.forEach(function (folderId) {
        work = work.then(function () {
          return mutate({ type: "folderDelete", id: folderId });
        });
      });
      work.then(function () {
  ...
  function renderActions() {
    ...
    var counts = [];
    if (picked.length) counts.push(picked.length + " selected");
    if (pickedFolders.length) {
      counts.push(pickedFolders.length +
        (pickedFolders.length === 1 ? " folder selected" : " folders selected"));
    }
  ...
  // Bands render under a filter too, so this syncs whatever headers are on
  // screen. Gating it on All would leave a filtered band's checkbox stale.
  ...
  function refresh() {
    return sendToWorker({ type: "savedGet" }).then(function (res) {
      ...
      // The folder selection is pruned the same way and in the same place: a
      // folder deleted under us must not sit in the delete set.
