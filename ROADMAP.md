...
- Listing updates at upload time: privacy policy URL on GitHub Pages,
  ...
  toolbar popup. The screenshots and the README are done: nine 1280x800
  shots regenerated by pipeline/make_screenshots.py. Screenshot rule for
  future scenes: vary the featured characters across the set (the set
  once fronted 學 in three shots; shots 1 and 9 now feature 天 and 樂,
  and 學生 stays only in the Japanese shot, where the variant note is
  the point).
